...
description: Use on demand when planning, designing, implementing, testing, or reviewing backend features that require Java with Spring Boot or Spring Framework code changes. Trigger for Spring Boot 3.x backend work such as REST controllers, services, DTOs, validation, Spring Data JPA repositories, transactions, Liquibase/database integration, Spring Security 6, WebFlux, Actuator, Spring Cloud, backend architecture, and Java API behavior. Do not use for frontend-only work, generic Java not involving Spring backend code, documentation-only edits, DevOps/infrastructure-only tasks, database-only SQL without Spring integration, high-level product planning with no backend design decisions, or simple questions that can be answered without loading Spring-specific implementation guidance.
license: MIT
metadata:
  version: "1.1.0"
  domain: backend
...
Load this skill only when the task needs Spring Boot or Spring Framework backend judgment while planning or changing Java code.

Use it for:
- Planning backend features that will be implemented in Java/Spring Boot.
- Implementing or modifying controllers, services, DTOs, repositories, entities, validation, exception handling, or configuration.
- Adding or changing Spring Security, authentication, authorization, CORS, method security, or token flows.
- Integrating persistence, migrations, transactions, Testcontainers, or database behavior through Spring.
- Designing Spring module boundaries, REST API behavior, service contracts, or backend tests.
- Reviewing Java/Spring backend code for correctness, maintainability, layering, security, or test coverage.

Do not load it for:
- Frontend-only work, UI changes, client-side routing, CSS, or JavaScript/TypeScript tasks.
- DevOps-only work such as Docker, CI, deployment scripts, or cloud resources unless Spring runtime behavior or configuration is part of the task.
- Database-only schema or SQL work that does not touch Spring entities, repositories, migrations, or transaction behavior.
- Documentation-only edits, README updates, prompt changes, or general project planning with no backend implementation decisions.
- Generic Java questions that do not involve Spring Boot, Spring Framework, or backend feature design.
- Quick terminal lookups, dependency version checks, or simple explanations that do not benefit from Spring-specific workflow guidance.
