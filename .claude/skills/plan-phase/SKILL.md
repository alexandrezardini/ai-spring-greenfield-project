...
**Dispatch Context7 lookups in parallel.** At the start of plan drafting, issue one tool call per library mentioned in the decisions document, all in a single message. For example, for an auth phase that chose Spring Security, Spring Data JPA, Flyway/Liquibase, JWT support, and an email library/provider, send one `mcp__context7__query-docs` call per library/framework in parallel. Do NOT look up libraries one-by-one as you write each SI — batch them upfront and refer back to the results.
...
| File | Layer | Verifies |
|------|-------|----------|
| ExampleTest.java | Unit/Integration/E2E | what this file tests |
...
- Be specific enough that another agent can implement without ambiguity. "Set up rate limiting" is vague. "Configure a Spring MVC interceptor or Spring Security filter to apply a 10-requests-per-minute limit on auth endpoints" is actionable.
- Include names of libraries, Spring modules/starters, patterns, and relevant configurations. The plan must reflect the user's technical decisions.
- Do not include code. The plan describes what to do, not how to code it. Referencing library APIs, Spring annotations, configuration properties, method names, and Maven coordinates in prose is expected.
- When introducing new dependencies, list the Maven coordinates with a version compatible with the project's existing Java and Spring Boot versions. Example: `Add org.springframework.boot:spring-boot-starter-security managed by the Spring Boot parent/BOM` or `Add io.jsonwebtoken:jjwt-api:<compatible-version> plus its runtime modules`. This prevents dependency and version conflicts during implementation.
...
E2E tests are listed per endpoint group (one row per E2E test class — typically the set of endpoints sharing a controller/feature), not per individual endpoint. If the SI creates no testable artifacts, the Tests section may be omitted.
...
- SIs that introduce new dependencies must list Maven coordinates with compatible versions or indicate that the dependency is managed by the Spring Boot parent/BOM.
- When a phase spans multiple subprojects, repeat the test/compilation/build deliverables per subproject, each with its own command (e.g., `All SI tests pass in stream-tube-backend (./mvnw test)` and `Project builds successfully in stream-tube-backend (./mvnw clean package)`).
