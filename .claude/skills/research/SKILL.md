...
1. **Check the relevant project's installed versions first.** Identify which Java/Maven project in the monorepo is in scope (e.g., `stream-tube-backend/` or another Spring Boot backend project) and check its `pom.xml` to see the Java version, Spring Boot version, Maven-managed dependencies, plugins, and versions already declared. If the project uses the Maven Wrapper, prefer `./mvnw` for any dependency or build-related verification. This constrains which documentation versions to fetch and which alternatives are truly compatible with the existing stack.
