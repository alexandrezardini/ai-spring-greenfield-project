...
- **Branch check**: `git status` and `git branch --show-current`. If the current branch is `main` or `dev`, or if there are uncommitted changes touching files outside the target subproject's directory (e.g., outside `stream-tube-backend/` when that is the target), stop and ask the user to set up the right branch first.
- **Subproject readiness**: the target subproject exists and is a valid Java/Maven project (e.g., `stream-tube-backend/pom.xml` present, and Maven Wrapper files such as `mvnw`/`.mvn/` present when the project uses the wrapper). If not, ask the user to set it up first.
...
Re-read the SI section in full (Description, Technical actions, Tests, Dependencies, Acceptance criteria). Load relevant best-practices skills matching the artifacts this SI builds (e.g., `spring-boot-engineer` for controllers/services/configuration, `jpa-pattern` for entities/repositories/migrations, `java-spring-boot-best-practices` for Java/Spring conventions) — load only what this SI needs, not all available skills. Keep a short internal checklist for this SI: one item per technical action; additionally, when the SI has a Tests section, one item per test file plus a "run tests" item. This is working memory to keep the SI on track — not a formal deliverable.
...
2. **Compilation check**: Run the compilation command defined in the plan's Deliverables (e.g., `./mvnw test` or `./mvnw -DskipTests compile` for Maven projects).
3. **Project build**: Run the build command defined in the project (e.g., `./mvnw clean package`) to verify the application compiles and packages correctly.
...
If the phase spans multiple subprojects, the plan's Deliverables checklist will list these checks per subproject (e.g., `All SI tests pass in stream-tube-backend` and `Project builds successfully in stream-tube-backend`) — run each subproject's commands independently, in the order implied by the checklist.
