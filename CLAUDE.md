...
- `stream-tube-backend/` — Backend API (Spring Boot Application). Contains modules for users, channels, videos, comments, etc.
...
- **API** (Spring Boot Application) → business rules, auth, reads/writes DB, uploads to storage, publishes jobs to queue, sends emails
